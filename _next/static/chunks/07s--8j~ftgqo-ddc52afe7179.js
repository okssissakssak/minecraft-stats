...
/* 표본이 적을 때 값을 기준선 쪽으로 당긴다(축소추정).
   안 하면 리필 기회가 0인 신규 유저가 "연쇄 0% · 캐릭터 대비 -36%p" 로 찍힌다 —
   못한 게 아니라 잰 적이 없는 건데 최하위로 깔린다. n 이 커지면 원래 값으로 수렴한다. */
var _MXSHRINK = 20;
function hcRpShrink(obs, n, base) { return n > 0 ? (obs * n + base * _MXSHRINK) / (n + _MXSHRINK) : base; }
...
function hcReportAll(stats, kills) {
	...
	A.players.forEach(function (p, nick) {
		...
		eWpa = eWpa === null ? 0 : eWpa; eRef = eRef === null ? 0 : eRef; eOpn = eOpn === null ? 0 : eOpn;
		o.roleAdjusted = eKpr > 0 ? hcRpShrink(o.kpr, R, eKpr) / eKpr : 1;
		o.characterAdjustedWpa = hcRpShrink(o.wpaPerRound, R, eWpa) - eWpa;
		// 화면에 그대로 뜨는 값이라 이쪽도 당긴다. 안 하면 "연쇄 0% · 캐릭터 대비 -1.7%p" 처럼
		// 두 숫자가 서로 어긋난 채 나온다.
...
/* 주력 캐릭터 카드 */
function hcRpTopChars(o, meta, chars, gmix) {
	...
	return rows.slice(0, 3).map(function (r) {
		var ck = p.charKills.get(r.name) || { a: 0, s: 0, m: 0 }, t = ck.a + ck.s + ck.m,
			md = meta && meta.get ? meta.get(r.name) : null;
...
/* 한 명분 리포트 완성 */
function hcReportOf(nick, stats, kills, meta, luck) {
	var all = hcReportAll(stats, kills), o = all.players.get(nick);
	if (!o) return null;
	o.actualTier = hcRpTierName(o.actualPoint);
	// 킬로그(경기 #334~)가 한 라운드도 없으면 교전 지표를 만들 수 없다.
	// 없는 값을 0 으로 채워 보여주면 "개인 실력 지수 0.0 · 전체 상위 100%" 같은 거짓말이 된다.
	// 지표는 못 만들되 화면은 띄우고, 만들 수 있는 것(판수·승률·K/D·티어)만 보여준다.
	o.noLog = !(o.logRounds > 0);
...
return a}
function hcMxSection(H,eg,kills,meta){
...
(0,s.jsx)("span",{children:p.noLog?"킬로그 없는 구간":"킬로그 "+p.logMatches+"경기 분석 · 전체 유저 대비 백분위"})]});
// 킬로그가 없는 유저 — 만들 수 있는 것만 보여주고 왜 없는지 밝힌다.
if(p.noLog)return(0,s.jsxs)(s.Fragment,{children:[head,
(0,s.jsxs)("div",{className:"mx-wrap",children:[
(0,s.jsxs)("div",{className:"mx-summary",children:[
(0,s.jsxs)("div",{children:[
(0,s.jsx)("small",{children:"총 경기"}),
(0,s.jsx)("strong",{children:p.totalGames}),
(0,s.jsxs)("span",{children:[p.wins,"승 ",p.totalGames-p.wins,"패"]})]}),
(0,s.jsxs)("div",{children:[
(0,s.jsx)("small",{children:"실제 티어"}),
hcMxTier(p.actualPoint,p.actualTier,!0),
(0,s.jsx)("span",{children:"최근 경기 기준"})]}),
(0,s.jsxs)("div",{children:[
(0,s.jsx)("small",{children:"승률"}),
(0,s.jsxs)("strong",{children:[p.winRate.toFixed(1),"%"]}),
(0,s.jsxs)("span",{children:["평균 ",p.avgKills.toFixed(1),"킬"]})]}),
(0,s.jsxs)("div",{children:[
(0,s.jsx)("small",{children:"K / D"}),
(0,s.jsx)("strong",{children:p.kd.toFixed(2)}),
(0,s.jsx)("span",{children:"전 경기 누적"})]})]}),
p.playstyleTags.length?(0,s.jsx)("div",{className:"mx-tags",children:p.playstyleTags.map(function(t){
return(0,s.jsx)("span",{className:"mx-tag"+("매칭주작 피해자"===t?" mx-tag-bad":" mx-tag-good"),children:t},t)})}):null,
(0,s.jsx)("p",{className:"mx-note",children:"교전 지표(육각 차트·진단)는 킬 기록이 있어야 만들 수 있습니다. 이 유저의 경기는 킬 기록 수집이 시작된 경기 #334 이전이라 아직 만들 수 없습니다."})]})]});
var all=hcReportAll(stats,kills),
vals=_MXDEFS.map(function(d){return hcMxQ(all,p,d)});
return(0,s.jsxs)(s.Fragment,{children:[head,
(0,s.jsxs)("div",{className:"mx-wrap",children:[
(0,s.jsxs)("div",{className:"mx-summary",children:[
(0,s.jsxs)("div",{children:[
...
p.logRounds<30?(0,s.jsxs)("p",{className:"mx-note",children:["킬 기록 ",p.logMatches,"경기 · ",p.logRounds,"라운드로만 계산한 값이라 흔들릴 수 있습니다."]}):null,
